                ...
                BEGIN
                    INSERT  INTO dbo.Orders_status_Audit
						   ...
						   DECLARE @NewDescription                nvarchar(4000);
                            ...
                            IF @NewName <> @OldName 
							   begin
                                SET @ChangeDescription = '' + isnull(@ChangeDescription,'') + '  TypeItemName = Old ->"' +  ISNULL(@OldName,'') + ' " NEW -> " ' + isnull(@NewName,'') + '", ';
							   end
                            IF @NewSysTypeOrderStatusName <> @OldSysTypeOrderStatusName
							   begin
                                SET @ChangeDescription = '' + isnull(@ChangeDescription,'') + '  SysTypeItemName = Old ->"' + ISNULL(@OldSysTypeOrderStatusName,'') + ' " NEW -> "' + ISNULL(@NewSysTypeOrderStatusName,'') + '", ';
							   end
                            IF @NewDescription <> @OldDescription
							   begin
                                SET @ChangeDescription = '' + isnull(@ChangeDescription,'') + '  Description = Old ->"' + ISNULL(@OldDescription,'') + ' " NEW -> " ' + ISNULL(@NewDescription,'') + '", ';
                               end
                            SET @ChangeDescription = 'Updated: ' + ' TypeItem = (' +  isnull(cast(@OldId_Status as nvarchar(20)),'')+ ') ' + @ChangeDescription
        ...
        BEGIN
            INSERT  INTO dbo.Orders_status_Audit
                    ...
                    SET @ChangeDescription = 'Inserted: '
                                         + 'Id_TypeItem="' + CAST(@Id_Status_3 AS NVARCHAR(20)) + '" ';
